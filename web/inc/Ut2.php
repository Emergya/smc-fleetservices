   ...
   function add($datos=array())
	{
		...
		if(intval($costeXdia)==0.00) 
		{
		    Errores::g()->add('Debe especificar el coste al dia');
			return false;
		}
        
        if(intval($costeXkm)==0.00) 
        {
        ...
        {	
    		...
    		$c="select id_transportes from transportes where nombre='$nombre' and id_usuario=$idUser limit 1";				
    		if($this->bd->put($c))
    		{
    
    		    if($this->bd->num()!=0)
    			{
    			    $nombre=ucfirst(strtoupper($nombre)); // para comparar en base de datos y registrar
   ...
   function edit($id=0,$datos=array())
    {
        ...
        {   
            ...
            $c="select id_transportes from transportes where nombre='$nombre' and id_transportes<>$id limit 1";
            if($this->bd->put($c))
            {
